...
def evaluate(config_path: Path, checkpoint: Path, output_dir: Path, aed_root: Path, split_dir: Path, device, cfg: dict | None = None) -> dict:
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(loader)):
            ...
            image_rows.append({"index": idx, "name": batch["name"][0], "panel": str(panel_path)})
...
def main() -> int:
    ...
    evaluate(Path(args.config), Path(args.checkpoint), Path(args.output_dir), Path(args.aed_root).resolve(), Path(args.split_dir).resolve(), device)
    return 0
